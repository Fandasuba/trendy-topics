...
import { googleTrends } from "../pages/API/api"
...
interface optionsObject {
    keyword?: string;  // Optional string of keyword
    startTime: Date;   // Required start date
    endTime: Date;     // Required end date
    geo: string;       // Required country code
    hl: string;        // Required language code
    timezone?: number; // Optional timezone
    category: number;  // Required category number
    property: string;  // Required property type
    resolution?: string; // Optional resolution
    granularTimeResolution?: boolean; // Optional time resolution flag
}
const optionsObject = {  
}
...
useEffect(() => {
    if (submit){
    googleTrends.apiMethod(optionsObject)
    setSubmit(false)
}
}, [submit]
)
